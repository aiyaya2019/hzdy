...
use app\common\model\SpikeTime;
...
class Spike extends Common
{
    ...
    public function index()
    {
        if(request()->isPost()){
            ...
            $time = time();
            $time_id = SpikeTime::where(''.$time.' < end_time')->column('id');

            if (count($time_id) > 0){
                $time_id = implode(',',$time_id);
                $post['field'] = 'id,title,pic,price,old_price,add_time,time_id';
                $post['with'] = 'time';
                $post['count'] = 10;
                $post['where'] = 'time_id in('.$time_id.')';
                $list = $this->selectAll('spike_goods',$post);
            }else{
                return_ajax(400,'暂无秒杀');
            }
    ...
    public function details()
    {
        if(request()->isPost()){
            ...
            $list = $this->getFind('spike_goods',$post);
    ...
    public function addorder()
    {
        if(request()->isPost()){
            ...
            //查询库存是否充足
            $stock = SpikeGoods::where('id',$post['id'])->value('stock');
            ...
            if ($post['pay_type'] == 1){
                //获取
                $post['field'] = 'id,title,pic,price,old_price,fare_price';
                $post['where']['id'] = $post['id'];
                $list = $this->getFind('spike_goods',$post);
                return_ajax(200,'获取成功',$list);
            }else{
                ...
                $post['order_sn'] = order_sn('spike_order');
                $post['uid'] = $this->uid;
                $post['order_price'] = $post['goods_price'];
                $post['pay_money'] = sprintf('%.2f',$post['goods_price'] + $post['fare_price']);
                $post['goods_id'] = $post['id'];
                unset($post['id']);
                $res = save('spike_order',$post,true);
                if ($res){
    ...
    public function spikepay()
    {
        if(request()->isPost()){
            ...
            $url = 'https://hzdy.kailly.com/api/Weix/spike_callback';
            $wexxin = controller('Weix');
            $res = $wexxin->wxpay('订单支付',$post['order_sn'],0.01,$openid,$url,'spike_order');
    ...
    public function money()
    {
        if(request()->isPost()) {
            ...
            if ($post['money'] > $money){
                ...
                //查询订单信息
                $order = SpikeOrder::get(['order_sn'=>$post['order_sn']]);
                ...
                if($order['status'] == 1){
                    ...
                    if($order->save()){
                        ...
                        controller('Weix')->send($order['order_sn'],4,1);
    ...
    public function getorder()
    {
        if(request()->isPost()){
            ...
            $data = SpikeOrder::field('id,order_sn,goods_title,goods_pic,goods_price,status,add_time,type,fare_price')->where('uid',$this->uid)->where($where)->select();
            if ($data){
    ...
    public function delete_order()
    {
        if(request()->isPost()){
            $post = input('post.');
            if (empty($post['id'])) { return_ajax(400,'缺少必传参数id'); }
            $res = SpikeOrder::where(['id'=>$post['id'],'uid'=>$this->uid])->delete();
            if ($res){
    ...
    public function order_detail()
    {
        if(request()->isPost()){
            $post = input('post.');
            if (empty($post['id'])) { return_ajax(400,'缺少必传参数id'); }
            $res = SpikeOrder::where('id',$post['id'])->find();
            if ($res){
                return_ajax(200,'获取成功',$res);
            }else{
    ...
    public function confirm_order()
    {
        if(request()->isPost()){
            $post = input('post.');
            if (empty($post['id'])) { return_ajax(400,'缺少必传参数id'); }
            $res = SpikeOrder::where(['id'=>$post['id'],'uid'=>$this->uid])->setField('status',4);
            if ($res){
        ...
        if(request()->isPost()){
            ...
            if (!empty($post['money'])){
                if ($post['money'] > $post['goods_price']){
                    return_ajax(400,'申请金额不能大于实际支付金额');
                }
            }
            $post['uid'] = $this->uid;
           
            $res = save('spike_order_refund',$post,true);
            if ($res){
                SpikeOrder::where('id',$post['order_id'])->setField('status',5);
