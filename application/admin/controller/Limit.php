...
Class Limit extends Common{
    ...
    public function index()
    {
        if(request()->isAjax()){
            ...
            $post['field'] = 'id,title,pic,price,old_price,fare_price,pay_num,stock,add_time,discount,time_id';
            $post['with'] = 'time';
    ...
    public function save()
    {
        ...
        if(request()->isPost()){
            ...
            $this->saves('limit_goods',$post);
        }

        $goods = $this->getFind('limit_goods',$id);
        //获取时间段
//        $data['where']['end_time'] = ['gt',time()];
//        $time = $this->selectAll('limit_time',$data);
        //商品标签
...
//            'time' => $time['list'],
    ...
    public function time()
    {
        if(request()->isAjax()){
            ...
            $list = $this->selectAll('limit_time',$post);
    ...
    public function time_save()
    {
        ...
        if(request()->isPost()){
            ...
            $this->saves('limit_time',$post);
        }

        $list = $this->getFind('limit_time',$id);
    ...
    public function order(){
        if(request()->isPost()){
            ...
            $list = $this->selectAll('limit_order',$post);
            return_ajax(200,'获取成功',$list['list'],$list['count']);
        }
        return $this->fetch();
    }
    ...
    public function refund(){
        if(request()->isPost()){
            ...
            $post['with'] = 'order';
            $list = $this->selectAll('limit_order_refund',$post);
            return_ajax(200,'获取成功',$list['list'],$list['count']);
        }
        return $this->fetch();
    }
    ...
    public function refund_info()
    {
            ...
            $id = input('id');
            $data = LimitOrderRefund::with('order')->where('id',$id)->find();
    ...
    public function refundAudit(){
        ...
        $order = $this->getFind('limit_order_refund',$data);
        if ($order['money'] == 0){
        ...
        if($post['status'] == 1){
            if($order['pay_type'] == 1){
                ...
                if ($res['result_code'] != 'SUCCESS') {
                    ...
                    LimitOrder::where('id',$order['order_id'])->update(['status'=>6,'update_time'=>time()]);
                    LimitOrderRefund::where('id',$post['id'])->setField('status',1);
                    return_ajax(200,'退款成功');
                }
            }else{
                // 余额支付
                $rs1 = price(1,$order['uid'],$money,'限时打折订单退款');
                $rs2 = LimitOrder::where('id',$order['order_id'])->update(['status'=>6,'update_time'=>time()]);
                $rs3 = LimitOrderRefund::where('id',$post['id'])->update(['status'=>1]);
                if($rs1 && $rs2 && $rs3){
                    return_ajax(200,'退款成功');
                }else{
            ...
            // 拒绝退款
            $rs1 = LimitOrder::where('id',$post['order_id'])->update(['status'=>7,'update_time'=>time()]);
            // 更新退款记录
            $rs2 = LimitOrderRefund::where('id',$post['id'])->update(['status'=>2]);
            if($rs1 && $rs2){
    ...
    public function order_excel()
    {
        ...
        $order = LimitOrder::order('id desc')->where($where)->select();
    ...
    public function order_save(){
        ...
        if(request()->isPost()){
            ...
            if(empty($post['id'])){
                ...
                unset($post['id']);
                //编辑
                $res = LimitOrder::where('id', $id)->update($post);
                if($res){
        ...
        $data = LimitOrder::where('id',$id)->find();
        return $this->fetch('',[
            'data'  => $data,
        ]);
    }
