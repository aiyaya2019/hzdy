<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:83:"/klwl_web/klwl_pj/hzdy.kailly.com/public/../application/admin/view/limit/order.html";i:1572600127;s:73:"/klwl_web/klwl_pj/hzdy.kailly.com/application/admin/view/common/meta.html";i:1552356108;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div class="layui-fluid">
    ...
    <form action="<?php echo url('Limit/order_excel'); ?>" method="post">
...
<script>
    layui.use(['table','form','laydate'],function(){
        ...
        var form = layui.form;
        var url = "<?php echo url('Limit/order'); ?>";
...
<script type="text/html" id="barDemo">
  <a class="layui-btn layui-btn-xs" lay-event="edit" href="<?php echo url('Limit/info'); ?>?id={{d.id}}">查看</a>
   <a class="layui-btn layui-btn-xs open" url="<?php echo url('Limit/order_save'); ?>?id={{d.id}}">编辑</a>
  {{# if (d.status=== 1) { }}
          <a class="layui-btn layui-btn-xs updates"  data-confirm="true" table="limit_order" key="status" val="2" data-id="{{d.id}}" lay-event="status">确认付款</a>
          ...
          <a class="layui-btn layui-btn-xs layui-btn-warm updates"  data-confirm="true" table="limit_order" key="status" val="8" data-id="{{d.id}}" lay-event="status">关闭订单</a>
  {{# } }}
  {{# if(d.status == 2){ }}
  <a class="layui-btn layui-btn-xs open" width="35%" height="50%" data-title="订单发货" url="<?php echo url('Limit/delivery'); ?>?id={{d.id}}">发货</a>
  {{# } }}
  <a class="layui-btn layui-btn-danger layui-btn-xs odd" data-confirm="true" table="limit_order" key="is_deleted" val="2" data-id="{{d.id}}" lay-event="del">删除</a>
  </script>
