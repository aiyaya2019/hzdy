        ...
        <div class="layui-form" lay-filter="layuiadmin-form-admin" style="padding: 20px 30px 1px 0;">
            <form action="" class="layui-form">
                            ...
                            <?php if(is_array($type) || $type instanceof \think\Collection || $type instanceof \think\Paginator): if( count($type)==0 ) : echo "" ;else: foreach($type as $key=>$vo): ?>
                                <optgroup label="<?php echo $vo['title']; ?>">
                                    <?php if(is_array($vo['this']) || $vo['this'] instanceof \think\Collection || $vo['this'] instanceof \think\Paginator): if( count($vo['this'])==0 ) : echo "" ;else: foreach($vo['this'] as $key=>$v): ?>
                                        <option value="<?php echo $v['id']; ?>" <?php if($goods['type_id'] == $v['id']): ?>selected<?php endif; ?>><?php echo $v['title']; ?></option>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="old_title" value="<?php echo $goods['old_title']; ?>" autocomplete="off" placeholder="请输入内容" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">原价</label>
                    <div class="layui-input-block">
                        <input type="number" name="old_price" value="<?php echo $goods['old_price']; ?>" lay-verify="required"  autocomplete="off" placeholder="请输入内容" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">现价</label>
                    <div class="layui-input-block">
                        <input type="number" name="price" value="<?php echo $goods['price']; ?>" lay-verify="required"  autocomplete="off" placeholder="请输入内容" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">可用积分</label>
                    <div class="layui-input-block">
                        <input type="number" name="point" value="<?php echo $goods['point']; ?>" lay-verify="required"  autocomplete="off" placeholder="请输入内容" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">限购数量</label>
                    <div class="layui-input-block">
                        <input type="number" name="quota" value="<?php echo $goods['quota']; ?>" lay-verify="required"  autocomplete="off" placeholder="请输入内容" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">销量</label>
                    <div class="layui-input-block">
                        <input type="number" name="pay_num" value="<?php echo $goods['pay_num']; ?>" lay-verify="required"  autocomplete="off" placeholder="请输入内容" class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">库存</label>
                    <div class="layui-input-block">
                        <input type="number" name="stock" value="<?php echo $goods['stock']; ?>" lay-verify="required"  autocomplete="off" placeholder="请输入内容" class="layui-input">
                    </div>
                </div>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="radio" name="use_coupon" value="1" title='是' <?php if($goods['use_coupon'] == 1 OR empty($goods)): ?>checked<?php endif; ?>>
                        <input type="radio" name="use_coupon" value="2" title='否' <?php if($goods['use_coupon'] == 2): ?>checked<?php endif; ?>>
                    </div>
                </div>
            ...
            <?php if(!empty($zone)): ?>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                    <?php if(is_array($zone) || $zone instanceof \think\Collection || $zone instanceof \think\Paginator): if( count($zone)==0 ) : echo "" ;else: foreach($zone as $key=>$vo): ?>
                        <input type="checkbox" name="zone_id[]" value="<?php echo $vo['id']; ?>" title="<?php echo $vo['title']; ?>" <?php if(in_array($vo['id'],$zgoods)): ?>checked<?php endif; ?>>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                    </div>
                </div>
            <?php endif; if(!empty($label)): ?>
                <div class="layui-form-item">
            ...
            <?php endif; if(!empty($nav)): ?>
                    <div class="layui-form-item">
                        <label class="layui-form-label">所属栏目</label>
                        <div class="layui-input-block">
                            <?php if(is_array($nav) || $nav instanceof \think\Collection || $nav instanceof \think\Paginator): if( count($nav)==0 ) : echo "" ;else: foreach($nav as $key=>$vo): if($goods): ?>

                                    <input type="checkbox" name="nav_id[]" value="<?php echo $vo['id']; ?>" title="<?php echo $vo['name']; ?>" <?php if(in_array($vo['id'],$goods['nav_id'])): ?>checked<?php endif; ?>>
                                <?php else: ?>
                                    <input type="checkbox" name="nav_id[]" value="<?php echo $vo['id']; ?>" title="<?php echo $vo['name']; ?>">
                                <?php endif; endforeach; endif; else: echo "" ;endif; ?>
                        </div>
                    </div>
                <?php endif; ?>
...
<script>
    ...
    //表单上传
    fromUpload('form',"<?php echo url('Goods/save'); ?>",function(data){
        if(data.code == 200){
            if(data.data != ''){
                layer.msg('商品保存成功，前往添加商品规格',{time:3000},function(){
                    window.location.href = "<?php echo url('Goods/attr_spec'); ?>?id="+data.data
                });
            } else {
